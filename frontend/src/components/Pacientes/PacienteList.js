            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                ...
                <div className="w-full md:w-auto flex flex-col sm:flex-row items-center gap-2">
                    <div className="relative w-full sm:w-48">
                        <input type="text" name="search" placeholder="Buscar por nome ou email..." value={filters.search} onChange={onFilterChange} className="w-full pl-10 pr-4 py-2 border rounded-lg dark:bg-slate-700 dark:border-slate-600 dark:text-white" />
                    ...
                    <button onClick={() => navigate('/pacientes/novo')} className="w-full sm:w-auto flex-shrink-0 flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition transform active:scale-95">
                        ...
                        {isLoading ? (
                            ...
                            pacientes.map((paciente) => (
                                <tr key={paciente.id} className="hover:bg-gray-100 dark:hover:bg-slate-700/50 transition-colors">
                                    <td className="px-6 py-4"><Link to={`/pacientes/${paciente.id}`} className="font-medium text-gray-900 dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400">{paciente.nome}</Link></td>
